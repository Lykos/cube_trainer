...
describe 'new training session' do
  ...
  it 'allows to create a new commutator training session with algs', :skip_on_ci do
    ...
    before_next_sleep
    within('#training-group-actions') { click_link_or_button 'Next' }

    # Not adding any stats because drag and drop is buggy with Selenium.
    before_next_sleep
    within('#stats-group-actions') { click_link_or_button 'Submit' }
