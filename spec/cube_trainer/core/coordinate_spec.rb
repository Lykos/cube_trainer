...
describe Core::Coordinate do
  ...
  context 'for an uneven n' do
    ...
    it 'checks equivalence of coordinates appropriately' do
      ...
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n, 1, 0)).to be true
      # rubocop:enable Lint/UselessComparison
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n, 1, -7)).to be true
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::D, n, 1, 0)).to be false
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n, 1, -1)).to be false
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n + 2, 1, 0)).to be false
    end
    ...
    it 'does the right thing for a corner' do
      expect(described_class.from_indices(Core::Face::U, n, 0, 0).rotations).to be == [described_class.from_indices(Core::Face::U, n, 0, 0), described_class.from_indices(Core::Face::U, n, 0, -1), described_class.from_indices(Core::Face::U, n, -1, -1), described_class.from_indices(Core::Face::U, n, -1, 0)]
    end

    it 'does the right thing for a wing' do
      expect(described_class.from_indices(Core::Face::U, n, 0, 2).rotations).to be == [described_class.from_indices(Core::Face::U, n, 0, 2), described_class.from_indices(Core::Face::U, n, 2, -1), described_class.from_indices(Core::Face::U, n, -1, -3), described_class.from_indices(Core::Face::U, n, -3, 0)]
    end

    it 'does the right thing for a midge' do
      expect(described_class.from_indices(Core::Face::U, n, 0, 3).rotations).to be == [described_class.from_indices(Core::Face::U, n, 0, 3), described_class.from_indices(Core::Face::U, n, 3, -1), described_class.from_indices(Core::Face::U, n, -1, -4), described_class.from_indices(Core::Face::U, n, -4, 0)]
    end

    it 'does the right thing for an oblique' do
      expect(described_class.from_indices(Core::Face::U, n, 1, 2).rotations).to be == [described_class.from_indices(Core::Face::U, n, 1, 2), described_class.from_indices(Core::Face::U, n, 2, -2), described_class.from_indices(Core::Face::U, n, -2, -3), described_class.from_indices(Core::Face::U, n, -3, 1)]
    end
  end
  ...
  context 'for an even n' do
    ...
    it 'checks equivalence of coordinates appropriately' do
      ...
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n, 1, 0)).to be true
      # rubocop:enable Lint/UselessComparison
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n, 1, -6)).to be true
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::D, n, 1, 0)).to be false
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n, 1, -1)).to be false
      expect(described_class.from_indices(Core::Face::U, n, 1, 0) == described_class.from_indices(Core::Face::U, n + 2, 1, 0)).to be false
    end
    ...
    it 'gets the right solved position for wing FU' do
      ...
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::F, n, 0, 1)
    end

    it 'gets the right solved position for wing UF' do
      part = Core::Wing.for_face_symbols(%i[U F])
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::U, n, 0, n - 2)
    end

    it 'gets the right solved position for wing BD' do
      part = Core::Wing.for_face_symbols(%i[B D])
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::B, n, n - 1, 1)
    end

    it 'gets the right solved position for wing DB' do
      part = Core::Wing.for_face_symbols(%i[D B])
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::D, n, n - 1, n - 2)
    end

    it 'gets the right solved position for wing LB' do
      part = Core::Wing.for_face_symbols(%i[L B])
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::L, n, 1, n - 1)
    end

    it 'gets the right solved position for wing BL' do
      part = Core::Wing.for_face_symbols(%i[B L])
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::B, n, n - 2, n - 1)
    end

    it 'gets the right solved position for wing RB' do
      part = Core::Wing.for_face_symbols(%i[R B])
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::R, n, n - 2, n - 1)
    end

    it 'gets the right solved position for wing BR' do
      part = Core::Wing.for_face_symbols(%i[B R])
      expect(described_class.solved_position(part, n, 0)).to be == described_class.from_indices(Core::Face::B, n, 1, 0)
    end

    it 'gets the right solved positions for wing FU' do
      part = Core::Wing.for_face_symbols(%i[F U])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::F, n, 0, 1), described_class.from_indices(Core::Face::U, n, 0, 1)]
    end

    it 'gets the right solved positions for wing UF' do
      part = Core::Wing.for_face_symbols(%i[U F])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::U, n, 0, n - 2), described_class.from_indices(Core::Face::F, n, 0, n - 2)]
    end

    it 'gets the right solved positions for wing BD' do
      part = Core::Wing.for_face_symbols(%i[B D])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::B, n, n - 1, 1), described_class.from_indices(Core::Face::D, n, n - 1, 1)]
    end

    it 'gets the right solved positions for wing DB' do
      part = Core::Wing.for_face_symbols(%i[D B])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::D, n, n - 1, n - 2), described_class.from_indices(Core::Face::B, n, n - 1, n - 2)]
    end

    it 'gets the right solved positions for wing LB' do
      part = Core::Wing.for_face_symbols(%i[L B])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::L, n, 1, n - 1), described_class.from_indices(Core::Face::B, n, 1, n - 1)]
    end

    it 'gets the right solved positions for wing BL' do
      part = Core::Wing.for_face_symbols(%i[B L])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::B, n, n - 2, n - 1), described_class.from_indices(Core::Face::L, n, n - 2, n - 1)]
    end

    it 'gets the right solved positions for wing RB' do
      part = Core::Wing.for_face_symbols(%i[R B])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::R, n, n - 2, n - 1), described_class.from_indices(Core::Face::B, n, n - 2, 0)]
    end

    it 'gets the right solved positions for wing BR' do
      part = Core::Wing.for_face_symbols(%i[B R])
      expect(described_class.solved_positions(part, n, 0)).to be == [described_class.from_indices(Core::Face::B, n, 1, 0), described_class.from_indices(Core::Face::R, n, 1, n - 1)]
    end
  end
end
