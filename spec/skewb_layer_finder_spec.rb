...
describe SkewbLayerFinder do
  ...
  it 'should find an existing layer' do
    expect(layer_finder.find_layer(cube_state, 0).extract_algorithms).to be == [Algorithm.empty]
  end

  it 'should find a one move layer' do
    parse_skewb_move('U').apply_to(cube_state)
    expect(layer_finder.find_layer(cube_state, 1).extract_algorithms).to be == [parse_skewb_alg('U\'')]
  end

  it 'should find a two move layer' do
    parse_skewb_alg('U R').apply_to(cube_state)
    expect(layer_finder.find_layer(cube_state, 2).extract_algorithms).to be == [parse_skewb_alg('R\' U\'')]
  end
  ...
  it 'should find multiple solutions if applicable' do
    ...
    expect(layer_finder.find_layer(cube_state, 2).extract_algorithms).to be == [parse_skewb_alg('B\' L'), parse_skewb_alg('L B\''), parse_skewb_alg('U\' L')]
  end
end
