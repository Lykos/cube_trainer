...
module CubeTrainer
  ...
  class DisjointUnionLetterPairAlgSet < LetterPairAlgSet
    ...
    def hinter
      @hinter ||= DisjointUnionHinter.new(@alg_sets.map { |a| RestrictedHinter.new(a.input_items.map { |e| e.representation }, a.hinter) })
    end

    def generate_input_items
      @alg_sets.collect_concat { |a| a.input_items }
    end
