module CubeTrainer
  ...
  class WCAResult
    ...
    def self.multi(result_int)
      ...
      multi_type = case result_int / 1000000000
                   when 1 then :old
                   when 0 then :new
                   else
      ...
      if multi_type == :new
        ...
        time_centis = (result_int / 100) % 100000 * 100
        if time_centis == 99999 then time_centis = nil end
        d = result_int / 10000000 % 100
        difference = 99 - d
        solved = difference + missed
        attempted = solved + missed
        new(:success, time_centis, attempted, solved, nil)
      elsif multi_type == :old
        time_centis = result_int % 100000 * 100
        if time_centis == 99999 then time_centis = nil end
        attempted = result_int / 100000 % 100
        s = result_int / 10000000 % 100
        solved = 99 - s
        new(:success, time_centis, attempted, solved, nil)
      else
        raise
