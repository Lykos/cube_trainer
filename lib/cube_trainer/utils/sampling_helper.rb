module CubeTrainer
  ...
  module SamplingHelper
    ...
    def sample_by(array, &block)
      ...
      raise TypeError, "Negative weights are not allowed for sampling." unless weights.all? { |w| w.is_a?(Numeric) }
      raise ArgumentError, "Negative weights are not allowed for sampling." if weights.any? { |w| w < 0.0 }
