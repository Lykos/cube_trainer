  ...
  module Training
    ...
    class ResultHistory
      ...
      def initialize(
        ...
        raise TypeError unless cached_cases.all? { |c| c.respond_to?(:representation) }
      ...
      # Returns the representations of the last `num_items` results (including cached cases).
      def last_representations(num_items)
        if @max_num_items.nil? || num_items > @max_num_items
          @max_num_items = num_items
          @last_representations =
            @cached_cases.map(&:representation) +
            fetch_last_representations(num_items - @cached_cases.length)
        end

        adjusted_num_items = [num_items, @last_representations.length].min
        @last_representations[...adjusted_num_items]
      end
        ...
        @badness_averages ||=
          begin
            result = @mode.results
                          ...
                          .pluck(:representation, badness_array_exp).to_h
      ...
      def fetch_last_representations(num_items)
      ...
      def calculate_occurrence_days_since_last_hint(item)
        ...
        @mode
          .results
          .where(representation: item.representation)
      ...
      def last_occurrence_age_cache
        @last_occurrence_age_cache ||=
          begin
            ...
            @cached_cases.each { |i| result[i.representation] = 0 }
            result.freeze
          end
      end
      ...
      def occurrences_cache
        @occurrences_cache ||=
          begin
            result = @mode.results.group(:representation).count
            result.default = 0
            @cached_cases.each { |i| result[i.representation] += 1 }
            result.freeze
          end
      end
