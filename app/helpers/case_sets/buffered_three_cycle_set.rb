...
module CaseSets
  ...
  class BufferedThreeCycleSet < ConcreteCaseSet
    ...
    def row_pattern(refinement_index, casee)
      ...
      case_pattern(part_cycle_pattern(@part_type, *part_patterns), ignore_same_face_center_cycles: !(part_type < TwistyPuzzles::MoveableCenter))
    end
    ...
    def cases
      part_permutations =
        @part_type::ELEMENTS.permutation(2).select do |a, b|
          ...
          next false if a.solved_face == b.solved_face && a.face_symbols.length == 1 && b.face_symbols.length == 1
