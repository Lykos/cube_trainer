...
describe Algorithm do
  ...
  shared_examples 'correct cancellation algorithm' do |cube_size|
    it 'applies a rotation correctly to an algorithm' do
      property_of do
        ...
        modified_alg = Algorithm.move(rotation.inverse) + alg + Algorithm.move(rotation)
  ...
  it 'mirrors algorithms correctly' do
    expect(parse_algorithm("M' D D2 F2 D2 F2").mirror(Face::D)).to eq_cube_algorithm("M U' U2 F2 U2 F2")
    expect(parse_algorithm('2u 2f').mirror(Face::D)).to eq_cube_algorithm("2d' 2f'")
  end
  ...
  it 'mirrors Sarahs skewb algorithms correctly' do
    expect(parse_skewb_algorithm("F R'", sarah).mirror(Face::R)).to eq_sarahs_skewb_algorithm("L' B")
    expect(parse_skewb_algorithm("L F'", sarah).mirror(Face::R)).to eq_sarahs_skewb_algorithm("F' L")
    expect(parse_skewb_algorithm("B L'", sarah).mirror(Face::R)).to eq_sarahs_skewb_algorithm("R' F")
    expect(parse_skewb_algorithm("R B'", sarah).mirror(Face::R)).to eq_sarahs_skewb_algorithm("B' R")

    expect(parse_skewb_algorithm("F R'", sarah).mirror(Face::F)).to eq_sarahs_skewb_algorithm("R' F")
    expect(parse_skewb_algorithm("L F'", sarah).mirror(Face::F)).to eq_sarahs_skewb_algorithm("B' R")
    expect(parse_skewb_algorithm("B L'", sarah).mirror(Face::F)).to eq_sarahs_skewb_algorithm("L' B")
    expect(parse_skewb_algorithm("R B'", sarah).mirror(Face::F)).to eq_sarahs_skewb_algorithm("F' L")
  end
end
