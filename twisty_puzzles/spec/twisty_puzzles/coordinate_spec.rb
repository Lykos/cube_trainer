...
describe Coordinate do
  ...
  context 'for an uneven n' do
    ...
    it 'checks coordinates appropriately' do
      ...
      expect { described_class.from_indices(Face::U, n, 0, -7) }.not_to(raise_error)
      expect { described_class.from_indices(Face::U, n, 0, -8) }.to raise_error(ArgumentError)
      expect { described_class.from_indices(Face::U, n, 0, 7) }.to raise_error(ArgumentError)
      expect { described_class.from_indices(Face::U, n, 0, 0) }.not_to(raise_error)
      expect { described_class.from_indices(Face::U, n, 0, 5) }.not_to(raise_error)
      expect { described_class.from_indices(Face::U, n, 0, 6) }.not_to(raise_error)
    end
  ...
  context 'for an even n' do
    ...
    it 'checks coordinates appropriately' do
      ...
      expect { described_class.from_indices(Face::U, n, 0, -6) }.not_to(raise_error)
      expect { described_class.from_indices(Face::U, n, 0, -7) }.to raise_error(ArgumentError)
      expect { described_class.from_indices(Face::U, n, 0, 6) }.to raise_error(ArgumentError)
      expect { described_class.from_indices(Face::U, n, 0, 0) }.not_to(raise_error)
      expect { described_class.from_indices(Face::U, n, 0, 4) }.not_to(raise_error)
      expect { described_class.from_indices(Face::U, n, 0, 5) }.not_to(raise_error)
    end
    ...
    it 'gets the right solved position for wing FU' do
      ...
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::F, n, 0, 1)
    end

    it 'gets the right solved position for wing UF' do
      part = Wing.for_face_symbols(%i[U F])
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::U, n, 0, n - 2)
    end

    it 'gets the right solved position for wing BD' do
      part = Wing.for_face_symbols(%i[B D])
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::B, n, n - 1, 1)
    end

    it 'gets the right solved position for wing DB' do
      part = Wing.for_face_symbols(%i[D B])
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::D, n, n - 1, n - 2)
    end

    it 'gets the right solved position for wing LB' do
      part = Wing.for_face_symbols(%i[L B])
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::L, n, 1, n - 1)
    end

    it 'gets the right solved position for wing BL' do
      part = Wing.for_face_symbols(%i[B L])
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::B, n, n - 2, n - 1)
    end

    it 'gets the right solved position for wing RB' do
      part = Wing.for_face_symbols(%i[R B])
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::R, n, n - 2, n - 1)
    end

    it 'gets the right solved position for wing BR' do
      part = Wing.for_face_symbols(%i[B R])
      expect(described_class.solved_position(part, n, 0)).to eq_cube_coordinate(Face::B, n, 1, 0)
    end

    it 'gets the right solved positions for wing FU' do
      part = Wing.for_face_symbols(%i[F U])
      expect(described_class.solved_positions(part, n, 0)).to contain_exactly(described_class.from_indices(Face::F, n, 0, 1), described_class.from_indices(Face::U, n, 0, 1))
    end

    it 'gets the right solved positions for wing UF' do
      part = Wing.for_face_symbols(%i[U F])
      expect(described_class.solved_positions(part, n, 0)).to contain_exactly(described_class.from_indices(Face::U, n, 0, n - 2), described_class.from_indices(Face::F, n, 0, n - 2))
    end

    it 'gets the right solved positions for wing BD' do
      part = Wing.for_face_symbols(%i[B D])
      expect(described_class.solved_positions(part, n, 0)).to contain_exactly(described_class.from_indices(Face::B, n, n - 1, 1), described_class.from_indices(Face::D, n, n - 1, 1))
    end

    it 'gets the right solved positions for wing DB' do
      part = Wing.for_face_symbols(%i[D B])
      expect(described_class.solved_positions(part, n, 0)).to contain_exactly(described_class.from_indices(Face::D, n, n - 1, n - 2), described_class.from_indices(Face::B, n, n - 1, n - 2))
    end

    it 'gets the right solved positions for wing LB' do
      part = Wing.for_face_symbols(%i[L B])
      expect(described_class.solved_positions(part, n, 0)).to contain_exactly(described_class.from_indices(Face::L, n, 1, n - 1), described_class.from_indices(Face::B, n, 1, n - 1))
    end

    it 'gets the right solved positions for wing BL' do
      part = Wing.for_face_symbols(%i[B L])
      expect(described_class.solved_positions(part, n, 0)).to contain_exactly(described_class.from_indices(Face::B, n, n - 2, n - 1), described_class.from_indices(Face::L, n, n - 2, n - 1))
    end

    it 'gets the right solved positions for wing RB' do
      part = Wing.for_face_symbols(%i[R B])
      expect(described_class.solved_positions(part, n, 0)).to contain_exactly(described_class.from_indices(Face::R, n, n - 2, n - 1), described_class.from_indices(Face::B, n, n - 2, 0))
    end
